...
from third_party.intersect.cuda_lib import build_octree
...
def trilinear_interp(p, q, point_feats):
    ...
    point_feats = (weights * point_feats).sum(-2)
    return point_feats
...
def offset_points_faces(point_xyz, half_voxel=1, bits=2):
    c = torch.arange(1, 2 * bits, 2, device=point_xyz.device)

    ox, oy = torch.meshgrid([c, c])

    ox = ((ox - bits) / float(bits - 1)).reshape(-1, 1)
    oy = ((oy - bits) / float(bits - 1)).reshape(-1, 1)
    oz_plus = torch.ones_like(ox)
    oz_minus = -torch.ones_like(ox)

    offset_z_plus = torch.cat([ox, oy, oz_plus], 1)
    offset_z_minus = torch.cat([ox, oy, oz_minus], 1)

    offset_y_plus = torch.cat([ox, oz_plus, oy], 1)
    offset_y_minus = torch.cat([ox, oz_minus, oy], 1)

    offset_x_plus = torch.cat([oz_plus, ox, oy], 1)
    offset_x_minus = torch.cat([oz_minus, ox, oy], 1)

    offset = torch.cat(
        [offset_x_plus, offset_x_minus, offset_y_plus, offset_y_minus, offset_z_plus, offset_z_minus],
        0
    )

    return point_xyz.unsqueeze(1) + offset.unsqueeze(0).type_as(point_xyz) * half_voxel
...
def vertices_from_points(points, half_voxel):
    coords, _ = discretize_points(points, half_voxel)
    keys0 = offset_points(coords, 1.0).reshape(-1, 3)
    keys, vertices = torch.unique(keys0, dim=0, sorted=True, return_inverse=True)
    vertices = vertices.reshape(-1, 8)
    num_keys = torch.scalar_tensor(keys.size(0)).long()

    return keys, vertices, num_keys
...
def splitting_points(point_xyz, point_feats, values, half_voxel):
    ...
    print("Keys shape:", new_keys.shape)
    print("Keys idx shape:", new_keys_idx.shape)
    print("Vertices shape:", new_feats.shape)
    ...
    if values is not None:
        ...
        point_feats = F.embedding(point_feats, values).view(point_feats.size(0), 8, -1)
        print("P, Q, point_feats shape:", p.shape, q.shape, point_feats.shape)
