...
import builtins
import third_party.intersect.cuda_lib as _ext
...
class SparseVoxelOctreeRayIntersect(Function):
    @staticmethod
    def forward(ctx, voxelsize, n_max, points, children, ray_start, ray_dir, z_only=False):
        ...
        if H > N:
            temp_zeros = torch.zeros(ray_start.shape[:1] + (H - N,) + ray_start.shape[2:]).type_as(
                ray_start
            )

            ray_start = torch.cat(
                [ray_start, temp_zeros],
                1
            )
            ray_dir = torch.cat(
                [ray_dir, temp_zeros],
        ...
        inds, min_depth, max_depth = _ext.svo_intersect(
            ray_start.float(), ray_dir.float(), points.float(), children.int(), voxelsize, n_max, z_only
        )
