...
class DepthSubdivision(Subdivision):
    def __init__(
        ...
        if cfg.voxel_size is not None:
            ...
            self.max_hits = cfg.max_hits
            self.num_slices = cfg.max_hits
            self.voxel_size = (self.far - self.near) / (self.num_slices - 1)
...
class VoxelSubdivision(Subdivision):
    def __init__(
        ...
        if 'bootstrap' in cfg:
            ...
            if cfg.voxel_size is not None:
                ...
                voxel_size = (cfg.max_point[2] - cfg.min_point[2]) / cfg.max_hits + 1e-8
        ...
        if self.forward_facing:
            ...
            self.max_hits = int(np.round((max_point[2] - min_point[2]) / voxel_size)) * 3 + 1
    ...
    def update_iter(self, batch, batch_idx):
        system = self.get_system()
        batch_size = system.trainer.datamodule.cur_batch_size # noqa
