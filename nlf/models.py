...
class LightfieldModel(BaseLightfieldModel):
    def __init__(
        ...
        **kwargs
        ...
        self.use_skip = cfg.use_skip if 'use_skip' in cfg else False
        ...
        if self.use_skip:
            self.color_skip_pe = pe_dict[cfg.color_skip_pe.type](
                cfg.param.n_dims,
                cfg.color_skip_pe
            )
            self.pes += [self.color_skip_pe]

            color_model_in_channels += self.color_skip_pe.out_channels
    ...
    def forward(self, rays, **render_kwargs):
        ...
        if self.use_skip:
            if 'no_param' in render_kwargs and render_kwargs['no_param']:
                skip_rays = self.color_skip_pe(rays[..., :self.cfg.param.n_dims])
            else:
                skip_rays = self.color_skip_pe(self.param(rays[..., :6]))

            embed_rays = torch.cat([embed_rays, skip_rays], -1)
