...
def render_path_lightfield(c2w, up, xs, ys, focal):
    render_poses = []
    hwf = c2w[:,4:5]

    for y in ys:
        for x in xs:
            c = np.dot(c2w[:3,:4], np.array([x, y, 0, 1.]))
            z = normalize(c - np.dot(c2w[:3,:4], np.array([0,0,-focal, 1.])))
            render_poses.append(np.concatenate([viewmatrix(z, up, c), hwf], 1))
...
def load_llff_data(basedir, factor=8, recenter=True, bd_factor=.75, spherify=False, args=None, path_zflat=False):
