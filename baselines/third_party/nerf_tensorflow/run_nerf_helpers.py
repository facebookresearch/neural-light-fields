...
def get_epi_rays_horz(U, v, S, t, aspect, near=-1, far=0):
    us = np.meshgrid(
        np.linspace(-1, 1, U, dtype=np.float32),
        np.linspace(-1, 1, S, dtype=np.float32) / aspect,
        indexing='xy'
    )

    u = us[0]
    v = np.ones_like(us[0]) * v
    s = us[1]
    t = np.ones_like(us[0]) * t
